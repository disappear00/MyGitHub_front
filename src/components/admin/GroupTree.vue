  ...
  <ul class="space-y-1">
    <li v-for="node in nodes" :key="node.group_id">
      ...
      >
        ...
        <button
          class="flex-1 truncate text-left font-medium text-slate-800"
          type="button"
          @click="emit('select', node.group_id)"
        >
          {{ node.group_name }}
        </button>
        ...
        <button
          ...
          type="button"
          @click="emit('create-child', node.group_id)"
        >
          + 子分组
        </button>
      </div>
      ...
      <div
        v-if="node.children.length > 0 && expanded[node.group_id]"
        class="ml-6 mt-1 border-l border-slate-200 pl-3"
      >
        <GroupTree
          :nodes="node.children"
