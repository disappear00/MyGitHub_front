...
async function refreshAll() {
  ...
  try {
    const [u, g] = await Promise.all([
      userApi.list({ order_by_time: true }),
        ...
        <tbody class="divide-y divide-slate-200 bg-white">
          <tr v-for="u in users" :key="u.user_id" class="hover:bg-slate-50/60">
            ...
            <td class="px-4 py-3 text-sm text-slate-700">
              {{
                u.group_id
                  ? groupById.get(u.group_id)?.full_path ||
                    groupById.get(u.group_id)?.group_name ||
                    `#${u.group_id}`
                  : '—'
              }}
            </td>
  ...
  >
    <div class="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
      ...
      <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div v-else class="grid grid-cols-1 gap-3 sm:col-span-2 sm:grid-cols-2">
