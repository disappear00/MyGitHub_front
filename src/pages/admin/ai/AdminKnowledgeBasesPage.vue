...
function openEdit(kb: KnowledgeBaseResponse) {
  ...
  form.value = {
    ...
    group_id: kb.group_id ?? null,
    is_private: kb.is_private,
    configText: formatJson(kb.config ?? {}),
  }
  modalError.value = null
  modalOpen.value = true
}

function buildPayload(): { ok: true; value: KnowledgeBaseCreate } | { ok: false; error: string } {
...
async function onSave() {
  ...
  try {
    ...
    if (modalMode.value === 'create') {
      await knowledgeBaseApi.create(built.value)
    } else {
...
<template>
  <div class="flex items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-semibold text-slate-900">知识库管理</h1>
      <p class="mt-1 text-sm text-slate-600">对接后端 `/api/v1/knowledge-bases`</p>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="divide-y divide-slate-200 bg-white">
          <tr v-for="kb in items" :key="kb.kb_id" class="hover:bg-slate-50/60">
            <td class="px-4 py-3 text-sm text-slate-700">{{ kb.kb_id }}</td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-right text-sm">
              <RouterLink
                ...
                :to="`/admin/ai/knowledge-bases/${kb.kb_id}`"
              >
                详情
              </RouterLink>
              <button
              ...
              <button
                ...
                type="button"
                :disabled="!canDelete || deletingId === kb.kb_id"
                @click="onDelete(kb.kb_id)"
              >
                删除
              </button>
            </td>
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <div class="mt-1 text-sm text-slate-600">上传文档与检索调试在“详情”页</div>
        </div>
        <button
